...
class paymentService {
  ...
  try {
    ...
    if (booking.status !== 'provider_selected') {
      ...
      const serviceFee = Math.round((agreedPrice * this.platformFeePercentage) / 100);
...
if (booking.status === 'funds_released') {
    ...
    if (booking.status !== 'user_accepted_completion') {
      throw new Error('Booking completion must be accepted before releasing payment');
    }
  ...
  try {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          type: 'nuban',
          name: provider.accountName || `${provider.userId.firstName} ${provider.userId.lastName}`,
