  ...
  async reverseGeocodeGoogle(longitude, latitude) {
    ...
    return {
      ...
      components: {
        ...
        neighborhood:
          components.neighborhood || components.sublocality_level_1 || null,
        city:
          components.locality || components.administrative_area_level_2 || null,
...
───────────────────────────────────────────────────────────── */
  mergeAddresses(google, nominatim) {
    ...
    if (!google)
      return { formattedAddress: nominatim.formattedAddress, ...nominatim };
    if (!nominatim)
      return { formattedAddress: google.formattedAddress, ...google };
    ...
    const formattedAddress =
      parts.length >= 3 ? parts.join(", ") : google.formattedAddress; // fallback to Google's full string
  ...
  async getDirections(origin, destination, profile = "driving") {
    try {
      ...
      const response = await axios.get(
        `${this.googleBaseUrl}/directions/json`,
        {
          params: {
  ...
  buildDetailedAddress(context, fallback) {
    ...
    // Neighborhood / estate
    if (
      context.sublocality_level_1 &&
      context.sublocality_level_1 !== context.route
    ) {
      parts.push(context.sublocality_level_1);
    }
    if (
      context.neighborhood &&
      context.neighborhood !== context.sublocality_level_1
    ) {
    ...
    if (context.administrative_area_level_1)
      parts.push(context.administrative_area_level_1);
  ...
  async findNearbyProviders({
    ...
    try {
      const providers = await Provider.find({
        ...
        .select(
          "userId services currentLocation rating startingPrice completedJobs isAvailable",
        )
