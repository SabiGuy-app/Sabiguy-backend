...
class WalletService {
   ...
   */
  async recordPayment(userId, providerId, bookingId, breakdown) {
    try {
      const providerWallet = await this.getOrCreateWallet(providerId, 'Provider');
   ...
   */
  async releaseEscrow(providerId, amount, bookingId) {
    try {
  ...
  async withdraw(providerId, amount, bankDetails) {
    try {
      const wallet = await this.getOrCreateWallet(providerId, 'ServiceProvider');
  ...
  async fundUserWallet(userId, amount, paymentReference) {
    try {
...
// Pay from user wallet balance (instead of Paystack)
  ...
  async payFromWallet(userId, amount, bookingId) {
    try {
